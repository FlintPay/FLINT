/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CryptoInfoUpdateManyWithoutFileInfosInput } from "./CryptoInfoUpdateManyWithoutFileInfosInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
  IsInt,
  Min,
  Max,
  IsDate,
} from "class-validator";
import { Type } from "class-transformer";
import { FileContentUpdateManyWithoutFileInfosInput } from "./FileContentUpdateManyWithoutFileInfosInput";

@InputType()
class FileInfoUpdateInput {
  @ApiProperty({
    required: false,
    type: () => CryptoInfoUpdateManyWithoutFileInfosInput,
  })
  @ValidateNested()
  @Type(() => CryptoInfoUpdateManyWithoutFileInfosInput)
  @IsOptional()
  @Field(() => CryptoInfoUpdateManyWithoutFileInfosInput, {
    nullable: true,
  })
  cryptoInfos?: CryptoInfoUpdateManyWithoutFileInfosInput;

  @ApiProperty({
    required: false,
    type: () => FileContentUpdateManyWithoutFileInfosInput,
  })
  @ValidateNested()
  @Type(() => FileContentUpdateManyWithoutFileInfosInput)
  @IsOptional()
  @Field(() => FileContentUpdateManyWithoutFileInfosInput, {
    nullable: true,
  })
  fileContents?: FileContentUpdateManyWithoutFileInfosInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  fileName?: string | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  fileSize?: number | null;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  uploadDate?: Date | null;
}

export { FileInfoUpdateInput as FileInfoUpdateInput };
