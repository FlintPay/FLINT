...
const service = {
  ...
  fileInfos: () => FIND_MANY_RESULT,
  fileInfo: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("FileInfo", () => {
  ...
  test("GET /fileInfos/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/fileInfos"}/${nonExistingId}`)
  ...
  test("GET /fileInfos/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/fileInfos"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /fileInfos existing resource", async () => {
